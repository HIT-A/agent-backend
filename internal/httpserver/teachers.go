...
func handleTeacherSearch(opts Options) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		skill := skills.NewTeacherSearchSkill(registry)
...
func handleTeacherBatch(opts Options) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			w.Header().Set("Allow", http.MethodPost)
			writeJSONError(w, http.StatusMethodNotAllowed, "method not allowed")
		...
		skill := skills.NewTeacherBatchSearchSkill(registry)
		output, err := skill.Invoke(r.Context(), input, nil)
		if err != nil {
			writeInvokeError(w, err)
			return

...
def _api_key() -> str:
    return os.environ.get("BRAVE_API_KEY", "") or os.environ.get("BRAVE_SEARCH_API_KEY", "")
...
def _ddg_fallback(query: str, count: int, timeout: int = 20) -> List[Dict[str, str]]:
    ...
    for block in blocks:
        a = re.search(r'<a[^>]*class="result__a"[^>]*href="([^"]+)"[^>]*>(.*?)</a>', block, flags=re.S)
        if not a:
            continue
        ...
        sn = re.search(r'<a[^>]*class="result__snippet"[^>]*>(.*?)</a>', block, flags=re.S)
        if not sn:
            sn = re.search(r'<div[^>]*class="result__snippet"[^>]*>(.*?)</div>', block, flags=re.S)
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
    ...
    key = _api_key()
    if not key:
        return [TextContent(type="text", text=json.dumps({"error": "Missing BRAVE_API_KEY"}))]
